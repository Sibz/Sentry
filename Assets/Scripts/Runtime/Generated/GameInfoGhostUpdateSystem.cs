...
public class GameInfoGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<GameInfoSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<GameInfoComponent> ghostGameInfoComponentType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostGameInfoComponentArray = chunk.GetNativeArray(ghostGameInfoComponentType);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
...
#endif
                GameInfoSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostGameInfoComponent = ghostGameInfoComponentArray[entityIndex];
                ghostGameInfoComponent.Name = snapshotData.GetGameInfoComponentName(deserializerState);
                ghostGameInfoComponent.SizeX = snapshotData.GetGameInfoComponentSizeX(deserializerState);
                ghostGameInfoComponent.SizeY = snapshotData.GetGameInfoComponentSizeY(deserializerState);
                ghostGameInfoComponent.Id = snapshotData.GetGameInfoComponentId(deserializerState);
                ghostGameInfoComponentArray[entityIndex] = ghostGameInfoComponent;
            }
        }
    }
    [BurstCompile]
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<GameInfoSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<GameInfoComponent>(),
            },
        ...
        {
            All = new []{
                ComponentType.ReadOnly<GameInfoSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<GameInfoComponent>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<GameInfoSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<GameInfoSnapshotData>(true),
                ...
                ghostGameInfoComponentType = GetArchetypeChunkComponentType<GameInfoComponent>(),
...
public partial class GameInfoGhostSpawnSystem : DefaultGhostSpawnSystem<GameInfoSnapshotData>
{
}
